...
- `references/code-style.md` — the plugin's **fixed** code-quality baseline (naming, SOLID, comments-explain-why, shallow nesting). Not project-configurable; ships with shipwright and applies the same way everywhere.
- `references/file-organization.md` — the plugin's **fixed** file/folder separation-of-concerns rule (interfaces, constants, providers, services, controllers each get their own file). Also not project-configurable, and violations here are BLOCKING at review, not just a style nit.
...
- Smallest change that satisfies the spec's Acceptance Criteria. Match surrounding style and naming, within the fixed baseline in `references/code-style.md`: intent-revealing names, SOLID, comments for *why* only (default to none), early-return over deep nesting.
- **Before writing to any file, name which single responsibility-type it holds** (interface/type, constant, provider, service, controller, repository — see `references/file-organization.md`). The moment a second type wants into the same file — an inline interface in a controller, a magic constant in a service — stop and extract it to its own file/module instead of letting it accumulate. Follow the project's existing structural convention if one exists (NestJS module layout, Next.js App Router split, etc.); default to `references/file-organization.md`'s suggested per-responsibility split if none exists yet, and record that choice in `decisions.md` so later tickets in the same module match it.
